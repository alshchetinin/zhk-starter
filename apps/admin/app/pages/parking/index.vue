...
const { data, isPending } = useQuery(
  computed(() =>
    $orpc.parking.list.queryOptions({
      input: {
        page: page.value,
        pageSize,
...
function prefetch(id: string) {
  queryClient.prefetchQuery(
    $orpc.parking.getById.queryOptions({ input: { id } }),
  );
}
</script>
...
<template>
  <PageContainer>
    <div class="mb-6 flex items-center justify-between">
      <h1 class="text-2xl font-bold">Паркинги</h1>
      <UButton
    ...
    <UTable
      ...
      @select="(row: any) => router.push(`/parking/${row.original.id}`)"
    >
      <template #name-cell="{ row }">
        <span
          class="font-medium"
          @mouseenter="prefetch(row.original.id)"
