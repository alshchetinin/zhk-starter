...
const { data: item, isPending } = useQuery(
  computed(() => $orpc.storage.getById.queryOptions({ input: { id: id.value } })),
);

useHead({
  title: computed(() =>
    item.value ? `Кладовка ${item.value.name ?? ""}`.trim() : undefined,
  ),
});
...
<template>
  <PageContainer>
    <UBreadcrumb
      :items="[
        { label: 'Кладовки', to: '/storage', icon: 'i-solar-box-linear' },
    ...
    <template v-else-if="item">
      <div class="mb-6 flex items-center gap-3">
        <h1 class="text-2xl font-bold">Кладовка {{ item.name ?? '' }}</h1>
