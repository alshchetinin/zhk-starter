  ...
  <PageContainer>
    ...
    <template v-else-if="item">
      <div class="mb-6 flex items-center gap-3">
        <h1 class="text-2xl font-bold">Коммерция {{ item.name ?? '' }}</h1>
        <UBadge v-if="item.category" variant="subtle" color="neutral">{{ item.category }}</UBadge>
      ...
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div v-if="item.layoutImage" class="rounded-lg border border-(--ui-border) bg-(--ui-bg) p-6">
          <h3 class="mb-3 font-semibold">Планировка</h3>
          <img
            :src="item.layoutImage"
            :alt="item.name ?? 'Commerce'"
