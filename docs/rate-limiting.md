...
> **better-auth не падает без Redis.** `secondaryStorage`
> (`packages/auth/src/index.ts`) — fail-soft: `get/set/delete` глотают ошибки
> Redis (`get → null`, `set/delete → noop`). better-auth rate-limiter дёргает
> storage на каждом auth-запросе и не ловит исключения — без этого недоступный
> Redis возвращал бы `500` на любой вход/проверку сессии. Сессии при этом
> остаются в БД (drizzleAdapter), Redis — лишь вторичный кэш.
...
(`packages/auth/src/index.ts`), но **на том же Redis**:

```ts
rateLimit: {
  enabled: true,
  storage: "secondary-storage",   // тот же ioredis через secondaryStorage
...
`secondaryStorage` — ленивая обёртка над `getRedis()` из `@zhk/ratelimit`
(клиент создаётся при первом auth-запросе, не при импорте модуля).

Проверка лимита sign-in (после `RL_ENABLED` + живого Redis + сервера):
