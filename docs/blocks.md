...
хранятся как JSONB в БД (поле `blocks` у страниц/коллекций), редактируются в
...
```bash
# создать блок из JSON-конфига (основной путь для AI)
pnpm generate:block --config design/blocks/my-block.json

# или интерактивно
pnpm generate:block
```

Либо в админке в dev-режиме: **Разработка → Блоки** (`/dev/blocks`) — создание,
редактирование схемы полей и удаление блоков через UI. Изменения пишутся прямо
в исходники, Vite HMR подхватывает.
...
### 1. Dev-билдер `/dev/blocks` (основной)
...
### 2. CLI `pnpm generate:block`

```bash
# интерактивный мастер (@clack/prompts): имя, label, иконка, поля циклом
pnpm generate:block

# неинтерактивный — JSON-конфиг (основной путь для AI)
...
| `image` | `ImageUpload` (folder `blocks`) | `z.string().url()` | `string \| null` | optional → `.nullable()` (не `.optional()`); канонический default — `null` |
...
- Канонические default-значения: `""` для строковых, `0`, `false`, `[]`,
  `null` для image, первая опция для select. Если нужно другое — поле
  `default` (см. ниже).
