...
`fields` собирается на сервере из **стандартных колонок тикета** (Имя/Телефон/Email/Сообщение —
только непустые) с человекочитаемыми RU-лейблами. Контракт публичной формы при этом **не меняется**
(`public.tickets.create` принимает тот же input). Webhook получает весь `DeliveryContext` как JSON;
Telegram/email используют `fields` для аккуратного форматирования. Более богатые per-form поля (каждое
доп-поле модалки отдельной строкой) — будущее улучшение, требующее структурированного payload формы.
